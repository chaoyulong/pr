...
#include "../x264.h"
#include "../../common/malloc/my_malloc.h"
#include "../../common/printf/printf.h"
#include "../../common/math/math.h"


#define nanf(x)     ((float)nan(x))
...
static inline int isfinite(double val)
{
    unsigned int *pu=(unsigned int *)&val;
    if((((pu[1]&0x7ff00000)==0x7ff00000)&&(((pu[1]&0xfffff)!=0)||(pu[0]!=0))) ||  // 阶码全1,尾数不为0，不存在的数"NaN"
        (((pu[1]&0x7fffffff) ==0x7ff00000)&&(pu[0] == 0) ))        // 阶码全1,尾数部分全0，符号位为0表示正无穷; 阶码全1,尾数部分全0，符号位为1表示负无穷。      
    {
        return 0;
    }
    else
